...
for line in inputs:
    ...
    if start_x == stop_x:
        ...
        if start_x < stop_x:
            if start_y < stop_y:
                for delta_x,delta_y in zip(range(start_x, stop_x+1), range(start_y, stop_y+1)):
                    vent[delta_x][delta_y] += 1
            else:
                for delta_x,delta_y in zip(range(start_x, stop_x+1), range(start_y, stop_y-1, -1)):
                    vent[delta_x][delta_y] += 1
        else:
            if start_y < stop_y:
                for delta_x,delta_y in zip(range(start_x, stop_x-1, -1), range(start_y, stop_y+1)):
                    vent[delta_x][delta_y] += 1
            else:
